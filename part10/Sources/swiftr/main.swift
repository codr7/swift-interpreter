...
func path(_ parts: [String]) -> String {
    let ps = parts.filter { !$0.isEmpty }
    var p = URL(fileURLWithPath: ps[0])

    for pp in ps[1...] {
        p = p.appendingPathComponent(pp)
    }
...
class StandardLibrary: Namespace {
    ...
    init() {
        ...
        bindFunction("load", [("path", stringType)], nil) {(_, vm, stack, pos) throws in
            ...
            vm.currentPath = path([vm.currentPath, d])
            defer { vm.currentPath = previousPath }
            let startPc = vm.emitPc
            try load(vm, readForm, fromPath: path([previousPath, p]), inNamespace: self)
            vm.emit(.stop)
            try vm.evaluate(fromPc: startPc, stack: &stack)
        }
